...
class MDN(nn.Module):
    ...
    def __init__(self, p: int, num_hidden_layers=2, num_components=5, hidden_size=100, dropout_rate = 0.2,
                  distribution='gamma'):
        """
        Args:
    ...
    def quantiles(self, x: torch.Tensor, percentiles: list, l = None, u = None, max_iter = 1000, tolerance = 1e-7) -> torch.Tensor:
        ...
        """
        l = torch.Tensor([0]) if l is None else l #self.cutpoints[0] - (self.cutpoints[-1]-self.cutpoints[0]) if l is None else l
