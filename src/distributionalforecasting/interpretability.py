...
class KernelSHAP_DRN():
    ...
    """
    def __init__(self, explaining_data,
        ...
        sample_size = int(np.round(self.background_data_df_before_one_hot.shape[0] * nsamples_background_fraction))


        if self.other_shap_values is None:                 
            # Compute SHAP values for the DRN network
            kernel_shap_explainer = shap.KernelExplainer(self.value_function,\
                                                        shap.sample(self.background_data_df_before_one_hot,\
            ...
            if self.glm_value_function is not None:
                kernel_shap_explainer_glm = shap.KernelExplainer(self.glm_value_function,\
                                                                shap.sample(self.background_data_df_before_one_hot,\
                                                                    nsamples = sample_size))
                self.shap_values_kernel_glm = kernel_shap_explainer_glm(self.explaining_data)
    ...
    def shap_dependence_plot(self, features_tuple, output = 'value'):
        ...
        """
        tuple_indexes = [np.where(self.feature_names  == feature)[0][0] for feature in features_tuple if feature in self.feature_names]
        ...
        for feature in features_tuple:
            if instances[feature].dtype == object or instances[feature].dtype == 'category':
                    #not isinstance(instances[feature].values[0], float):
                encoder = sklearn.preprocessing.LabelEncoder()
                instances[feature] = encoder.fit_transform(instances[feature])
                encoders[feature] = encoder
                for class_label, encoding in zip(encoder.classes_, encoder.transform(encoder.classes_)):
        ...
        for feature_idx in range(2):
            shap_values = shap_for_tuple[:, feature_idx:(feature_idx+1)].reshape(batch_size, 1)
            ...
            colors = instances_for_tuple[:, color_feature_index:(color_feature_index+1)]

            ax = axes[feature_idx]
            scatter = ax.scatter(feature_values, shap_values, alpha=0.85, c=colors, cmap='viridis', s=22)
            ...
            if feature_name_tuple[color_feature_index] in encoders:
                ...
                cmap = mcolors.ListedColormap(plt.cm.viridis(np.linspace(0, 1, len(unique_vals))))
                norm = mcolors.BoundaryNorm(np.arange(-0.5, len(unique_vals) + 0.5, 1), cmap.N)
                ...
                cbar.set_label(f'{feature_name_tuple[color_feature_index]}', fontsize=30)
                ...
                cbar.set_label(f'{feature_name_tuple[color_feature_index]}', fontsize=30)  # Set the font size as needed
    ...
    def beeswarm_plot(self, features = None, output = 'value'):
        ...
        """
        features = self.background_data_df_before_one_hot.columns if features is None else features
        features_idx = [np.where(self.feature_names == feature)[0][0] for feature in features if feature in self.feature_names]
    ...
    def global_importance_plot(self, features = None, output = 'value'):
        ...
        feature_importance = sorted(zip(feature_names, shap_sum), key=lambda x: x[1], reverse=True)
    ...
    def shap_value_selection(self, indexes, output = 'value'):
        if output == 'value':
            ...
            if self.glm_value_function is None:
                ...
                shap_values = self.shap_values[:, indexes] + self.shap_values_kernel_glm.values[:, indexes] 
        return(shap_values)
...
class DRNExplainer:
    def __init__(
        ...
        self.default_cutpoints = default_cutpoints  # Cutpoints for feature partitioning.
        self.column_transformer = column_transformer  # Transformer for scaling numerical features.

        # Store the raw data and features information.
        self.background_data_df_before_one_hot = background_data_df_before_one_hot  # Data before one-hot encoding.
        self.cat_features = cat_features  # Categorical feature names.
        self.all_categories = all_categories  # Categories for all features if available.
        ...
        self.background_data_tensor = self._to_tensor(self.background_data_df)  # Convert the data to a PyTorch tensor.
    ...
    def plot_dp_adjustment_shap(
        self, instance_raw: pd.DataFrame, dist_property: str = 'Mean', 
        method: str = 'Kernel', nsamples_background_fraction: float = 1.0, 
        top_K_features: int = 3, adjustment: bool = True,
        other_df_models: list = None, model_names: list = None,
        cutpoints: list = None, num_interpolations: int = None, 
        labelling_gap: float = 0.05, synthetic_data = None, synthetic_data_samples: int = int(1e6),
        observation = None, plot_baseline: bool = True, 
        # Plotting parameters below:
        x_range: tuple = None, y_range: tuple = None, plot_y_label: str = None, 
        plot_title: str = None, density_transparency: float = 1.0, 
        shap_fontsize: int = 25, figsize: tuple = (20, 15), 
        label_adjustment_factor: float = 1.0, legend_loc: str = 'upper left'
    ):
        """
        ...
        lst_tensor = torch.linspace(lower_cutpoint, upper_cutpoint, num_interpolations).unsqueeze(-1)
        ...
        glm_pdf = np.exp(
                (self.glm.distributions(instance).log_prob(lst_tensor))
                .detach()
                .numpy())

        # DRN PDF is calculated from the probabilities of the distribution.
        drn_pdf =(self.drn.distributions(instance).prob(
                    lst_tensor)
                    .detach()
                    .numpy())
        ...
        index_of_closest_last = torch.argmin(torch.abs(lst_tensor - upper_cutpoint)).item()
        index_of_closest_first = torch.argmin(torch.abs(lst_tensor - lower_cutpoint)).item()

        # Baseline Density Plotting
        if plot_baseline:
            plt.plot(lst_tensor[index_of_closest_first:index_of_closest_last],\
                     glm_pdf[index_of_closest_first:index_of_closest_last],\
                     color='black', linewidth=3, label='Baseline Density', alpha = density_transparency)
        ...
        for i in range(index_of_closest_first, index_of_closest_last-1):
            ...
            plt.plot(grid_new, np.full_like(grid_new, drn_pdf_value), color='blue', linewidth=3, alpha = density_transparency)
            # Draw vertical dashed lines between the jumps
            if i < index_of_closest_last - 2:  # Check to avoid adding a vertical line at the end
                plt.vlines(end, drn_pdf[i],  drn_pdf[i+1], color='blue', linewidth=2, linestyles='dashed', alpha = density_transparency)

        # Add a label and plot DRN density if drn_pdf_value is greater than zero
        if drn_pdf_value > 0:
            plt.plot(grid_new, np.full_like(grid_new, drn_pdf_value), color='blue', linewidth=3, label='DRN Density', alpha=density_transparency)
        ...
        if synthetic_data is not None:
            ...
            y = np.array(synthetic_data(synthetic_data_samples, 0, specific_instance=instance_np)[1].values)
            print(instance, instance_np)
            if observation:
                # Plot vertical line for the observed property mean or quantile
                property_value = np.mean(y) if dist_property == 'Mean' else np.percentile(y, 90)
                axes.axvline(property_value, ls="dashed", alpha=0.75, linewidth=4,
                              label=f'True {dist_property}: {round(float(property_value), 3)}', color='orange')
            else:
                # Plot KDE for the synthetic data
                sns.kdeplot(y, color="orange",
                             label="True Density", gridsize=3000, linewidth=3, alpha=density_transparency)

        # Plot observed value if synthetic data is not available
        if observation is not None and synthetic_data is None:
            axes.axvline(observation, ls="dashed", alpha=0.75, linewidth=4,
                          label=f'Observation:\n{round(float(observation), 3)}', color='red')
        ...
        if other_df_models is not None:
            for idx, DF_model in enumerate(other_df_models):
                current_pdf = np.exp(DF_model.distributions(instance).log_prob(lst_tensor).detach().numpy())
                plt.plot(lst_tensor, current_pdf, label=f'{model_names[idx]} Density',
                          color=str(0.5 + 0.5 * idx / len(other_df_models)), alpha=density_transparency)
        ...
        if plot_title is not None:
            ...
            plt.title(f'{dist_property} {"Adjustment" if adjustment else "Explanation"}',
    ...
    def plot_adjustment_factors(self, instance, cutpoints = None, num_interpolations = None,\
        ...
        instance = self._to_tensor(self.one_hot_encoder(instance))\
                         if self.all_categories is not None else self._to_tensor(self.num_feature_std(instance))

        # If we are plotting concerning quantiles
        if percentiles is not None:
            lst_tensor = torch.linspace(cutpoints[0], cutpoints[-1], num_interpolations).unsqueeze(-1)
            print(lst_tensor[:5])
            cutpoints_label_bool = True
            print(self.glm.quantiles(instance, percentiles).detach().numpy())
            cutpoints = self.glm.quantiles(instance, percentiles).detach().numpy().squeeze(1)
        ...
        glm_pdf = np.exp(self.glm.distributions(instance).log_prob(lst_tensor).detach().numpy())
        ...
        plt.plot(lst_tensor[index_of_closest_first:index_of_closest_last],\
                  glm_pdf[index_of_closest_first:index_of_closest_last], color='black', linewidth=3, label='Baseline Density', alpha = density_transparency)
        ...
        # Add the label
        if drn_pdf_value > 0:
            plt.plot(grid_new, np.full_like(grid_new, drn_pdf_value), color='blue', linewidth=3, label='Adjusted Density', alpha = density_transparency)
        ...
        if other_df_models is not None:
            for idx, DF_model in enumerate(other_df_models):
                current_pdf = np.exp(DF_model.distributions(instance).log_prob(lst_tensor).detach().numpy())
                plt.plot(lst_tensor, current_pdf, label=f'{model_names[idx]} Density', color=str(0.5 + 0.5 * idx / len(other_df_models)), alpha = density_transparency)

        if plot_adjustments_labels:
            if cutpoints_label_bool:
                for i in range(len(cutpoints)):
                    if cutpoints[i] <= c_0 and cutpoints[i+1] > c_0:
                        plt.text(c_0, y_max/35, 'Adjusted $c_{0}$',ha='center', fontsize=14, color = 'black')
                        plt.axvline(c_0, ls="--", color='black', alpha=0.5)
                    elif cutpoints[i] >= lst_tensor[index_of_closest_first] and  cutpoints[i] <= lst_tensor[index_of_closest_last]:
                        plt.text(cutpoints[i], y_max * 0.8, f'Baseline\n {percentiles[i]}% Quantile',\
                                                                    ha='center', fontsize=20, color = 'black')
                        plt.axvline(cutpoints[i], ls="--", color='black', alpha=0.5)
                    elif cutpoints[i] >= c_K:
                        plt.text(c_K, y_max/35, 'Adjusted $c_{K}$',\
            ...
            for i in range(len(cutpoints)-1):
                if cutpoints[-1] <= self.default_cutpoints[0] or cutpoints[0] >= self.default_cutpoints[-1]:
                    region_start = cutpoints[0]
                    region_end =  cutpoints[-1]   
                    if not cutpoints_label_bool: 
                        plt.axvline(region_end, ls="--", color='black', alpha=0.5)
                        
                    plt.text((region_start + region_end) / 2, y_max/2, f'No Adjustment', ha='center', fontsize=16, color = 'black')
                    break
        
                # cutpoint_{i} < c_0 < cutpoint_{i +1}
                elif cutpoints[i] < self.default_cutpoints[0] and cutpoints[i+1] >= self.default_cutpoints[0]:
                        ...
                        if cutpoints[i+1] <= self.default_cutpoints[-1]: 
                            # [c_0, cutpoint_{i+1})
                            adjustment_idx = self.region_text(instance, interval_width, drn_pdf, glm_pdf, y_max,\
                                            region_start = self.default_cutpoints[0], region_end = cutpoints[i+1] ,\
                                                cutpoint_idx = i, adjustment_idx = 1,\
                                                cutpoints_label_bool = cutpoints_label_bool, percentiles = percentiles)
                        else: 
                ...
                elif cutpoints[i+1] >= self.default_cutpoints[-1] and cutpoints[i] >= self.default_cutpoints[0]:
                    # we need to break it down into [cutpoint_{i}, c_K) and [c_K, cutpoint_{i+1})
                    adjustment_idx = self.region_text(instance, interval_width, drn_pdf, glm_pdf, y_max,\
                                    cutpoints[i], self.default_cutpoints[-1],\
                                        i, adjustment_idx,\
                                        cutpoints_label_bool = cutpoints_label_bool, percentiles = percentiles)
                ...
                # c_0 < cutpoint_{i} < cutpoints_{i+1} < c_K
                    ...
                    adjustment_idx = self.region_text(instance, interval_width, drn_pdf, glm_pdf, y_max,\
                                    cutpoints[i], cutpoints[i+1] , i, adjustment_idx,\
                                                cutpoints_label_bool = cutpoints_label_bool, percentiles = percentiles)
       
        if synthetic_data is not None:
            instance_np = instance.detach().numpy()[0]
            y = np.array(synthetic_data(500000, 0, specific_instance = instance_np)[1].values)
            sns.kdeplot(y, color="red", label="True Density", gridsize = 3000, linewidth=3, alpha = density_transparency)
        ...
        if plot_mean_adjustment: 
            ...
            axes.axvline(DP_glm, ls="dashdot", alpha=0.9, linewidth=4,\
                        label = f'Baseline Mean:\n {round(float(DP_glm), 3)}', color='black')
            axes.axvline(DP_drn, ls="dashdot", alpha=0.9, linewidth=4,\
                        label = f'Adjusted Mean:\n {round(float(DP_drn), 3)}', color='blue')

        if synthetic_data is not None and plot_mean_adjustment: 
            axes.axvline(np.mean(y), ls="dashdot", alpha=0.9, linewidth=4,\
                        label = f'True Mean:\n {round(float(np.mean(y)))}', color='red')
        ...
        title_name = 'Density Plot with Adjustment Factors' if plot_adjustments_labels else 'Density Plot'
        if plot_title is not None:
            title_name = plot_title
        plt.title(title_name, fontsize=48 * label_adjustment_factor)


        plt.legend(prop={"size": 32 * (label_adjustment_factor-0.15)}, loc = "upper right")
        if x_range is not None:
            x_min, x_max = x_range
    ...
    def cdf_plot(self, instance, grid = None, cutpoints = None, \
        ...
        if other_df_models is not None:
            for idx, DF_model in enumerate(other_df_models):
                current_cdf = DF_model.distributions(instance).cdf(grid).detach().numpy()
                plt.plot(grid, current_cdf, label=f'{model_names[idx]} CDF', color=str(0.5 + 0.5 * idx / len(other_df_models)), alpha = density_transparency)

        #cdfs_mdn = mdn.distributions(torch.Tensor([0.5, 0.5]).reshape(1,2)).cdf(grid).detach().numpy()
        if synthetic_data is not None:
            samples = synthetic_data(n=500000, seed=1, specific_instance = instance.detach().numpy()[0])[1].values
            if dist_property == 'Mean':
                axes.axvline(np.mean(samples), ls="dashed", alpha=0.75, linewidth=4,\
                            label = f'True {dist_property}:{round(float(np.mean(samples)), 3)}', color='orange')
            else:
                axes.axvline(np.percentile(samples, 90), ls="dashed", alpha=0.75, linewidth=4,\
                            label = f'True {dist_property}:{round(float(np.percentile(samples, 90)), 3)}', color='orange')
        
        plt.plot(grid, cdfs_drn, label="DRN CDF", color = 'blue', linewidth = 3)
        if plot_baseline:
            plt.plot(grid, cdfs_glm, label="GLM CDF", color = 'black', linewidth = 3)
        if synthetic_data is not None:
            cdf_empirical = [self.empirical_cdf(samples, x) for x in grid.detach().numpy()]
    ...
    def kernel_shap_plot(
        ...
        if method == 'Kernel':
            shap_base_value_diff, shap_values_diff, feature_names = self.kernel_shap(
                explaining_data=instance_raw, distributional_property=dist_property,
                nsamples_background_fraction=nsamples_background_fraction, adjustment=adjustment
            ).shap_values_mean_adjustments()
            
            # Calculate distributional property values
            DP_glm, DP_drn = self._compute_distributional_properties(instance, dist_property, adjustment)
        else:
       ...
       # instance_values = np.insert(instance.detach().numpy()[0], 0, 1.000)
        ...
        if adjustment:
            axes.axvline(DP_glm, ls="dashed", alpha=0.75, linewidth = 4,\
                          label = f'Baseline {dist_property}:\n{round(float(DP_glm), 3)}', color='black') 
            
        # Plot vertical lines for means
        axes.axvline(DP_drn, ls="dashed", alpha=0.75, linewidth = 4,\
        ...
        for (idx,phi) in enumerate(phi_j_s_top):
            ...
            if idx < (len(phi_j_s_top)-1):
                plt.plot([phi_sum_next, phi_sum_next], [y_position, y_position+y_max*labelling_gap],\
                                         ls="--", color=color, alpha=0.8)
            #Draw the arrows
            axes.arrow(phi_sum_old, y_position,\
                             phi,\
                            0, head_width=y_max/20, head_length=gap/15, fc=color, ec=color, length_includes_head=True)
        ...
        if len(phi_j_s_remaining)>0:
            ...
            axes.arrow(phi_sum_next, y_position,\
                             phi_remaining_sum,\
                             0, head_width=y_max/20, head_length=gap/15, fc=color, ec=color, length_includes_head=True)
            plt.text(phi_sum_next, y_position+(y_max/(50)),\
    ...
    def _compute_distributional_properties(self, instance, dist_property, adjustment):
        ...
        if dist_property == 'Mean':
            ...
            DP_glm = self.glm.variance(instance).detach().numpy()[0] if adjustment else 0
            DP_drn = self.drn.distributions(instance).variance().detach().numpy()
        else:
            percentile = int(re.search(r'(\d+)% Quantile', dist_property).group(1)) if re.search(r'(\d+)% Quantile', dist_property) else None
            DP_glm = self.glm.quantiles(instance, [percentile]).detach().numpy()[0][0] if percentile and adjustment else 0
            DP_drn = self.drn.distributions(instance).quantiles([percentile]).detach().numpy()[0] if percentile else None
    ...
    def kernel_shap(self, explaining_data, distributional_property,\
                     adjustment = True, nsamples_background_fraction = 1.0,
                     glm_output = False, other_shap_values = None):
        """
        ...
        if distributional_property == 'Mean':
            value_function = lambda instances: self.mean_value_function(instances, adjustment=adjustment)
            glm_value_function = self.mean_glm
        elif distributional_property == 'Variance':
            value_function = lambda instances: self.variance_value_function(instances, adjustment=adjustment)
            glm_value_function = self.variance_glm
        else:
            value_function = self.set_value_function(distributional_property, adjustment, self.quantile_value_function)
            ...
            if match:
                ...
                glm_value_function = lambda instances: self.quantile_glm(instances, percentile=[percentile])
    ...
    def max_pdf_in_region(self, drn_pdf, glm_pdf, interval_width, cutpoint_idx):
        ...
        """
        drn_pdf_value = torch.max(torch.Tensor(drn_pdf[cutpoint_idx*interval_width:(cutpoint_idx+1)*interval_width]))
        glm_pdf_value = torch.max(torch.Tensor(glm_pdf[cutpoint_idx*interval_width:(cutpoint_idx+1)*interval_width]))
    ...
    def region_adjustments(self, instance, region_start, region_end):
        ...
        """
        region_end = region_end - 1e-03 if region_end == self.default_cutpoints[-1] else region_end
        factor = self.real_adjustment_factors(instance, [region_start, region_end]).detach().numpy()
        factor_background = np.mean(self.real_adjustment_factors(self.background_data_tensor,\
                                                 [region_start, region_end]).detach().numpy(), axis = 1)
    ...
    def region_text(self, instance, interval_width, drn_pdf, glm_pdf, y_max,\
                     region_start, region_end, cutpoint_idx, adjustment_idx, cutpoints_label_bool = False,
                          percentiles = None):
        """
        Text the density adjustment regions
        """
        max_of_two = self.max_pdf_in_region(drn_pdf, glm_pdf, interval_width, cutpoint_idx)
    ...
    def one_hot_encoder(self, instances):
        ...
        if self.cat_features:
            for feature in self.cat_features:
                instances[feature] = pd.Categorical(instances[feature], categories=self.all_categories[feature])
        
            instances = pd.get_dummies(instances, columns=self.cat_features).astype(float)
        
        if self.column_transformer is not None:
            background_df_one_hot = pd.get_dummies(self.background_data_df_before_one_hot, columns=self.cat_features).astype(float)
    ...
    def mean_drn(self, instances):
        ...
        """  
        instances = self._to_tensor(self.one_hot_encoder(instances)) \
                            if self.all_categories is not None else self._to_tensor(instances)
    ...
    def mean_value_function(self, instances, adjustment):
        ...
        """
        return self.mean_drn(instances) - self.mean_glm(instances) if adjustment\
    ...
    def quantile_drn(self, instances, percentile = [90], grid = None):
        ...
        """
        grid = torch.linspace(self.default_cutpoints[0], self.default_cutpoints[-1], 3000).unsqueeze(-1) if grid is None else grid

        instances = self._to_tensor(self.one_hot_encoder(instances)) \
                            if self.all_categories is not None else self._to_tensor(instances)
        
        return(self.drn.distributions(instances).quantiles(percentile, l = grid[0], u = grid[-1] * 1.5).detach().numpy())
    ...
    def quantile_glm(self, instances,  percentile = [90], grid = None):
        ...
        """
        grid = torch.linspace(self.default_cutpoints[0], self.default_cutpoints[-1], 3000).unsqueeze(-1) \
                                                                                if grid is None else grid

        instances = self._to_tensor(self.one_hot_encoder(instances)) \
                            if self.all_categories is not None else self._to_tensor(instances)
        return(self.glm.quantiles(instances, percentile, l = grid[0], u = grid[-1] * 1.5).detach().numpy().reshape(instances.shape[0], ))

    def quantile_value_function(self, instances, adjustment,\
                         grid = None,\
                        percentile = [90]):
        """
        ...
        return self.quantile_drn(instances, percentile, grid).reshape(instances.shape[0], ) -\
                         self.quantile_glm(instances, percentile, grid).reshape(instances.shape[0], ) if adjustment\
                                else self.quantile_drn(instances, percentile, grid).reshape(instances.shape[0], )
    
    def variance_glm(self, instances):

        raise NotImplementedError(f"Variance calculation for the GLM yet to be implemented!")
    
    def variance_drn(self, instances):

        raise NotImplementedError(f"Variance calculation for the DRN network yet to be implemented!")
    
    def variance_value_function(self, instances, adjustment):

        raise NotImplementedError(f"Kernel SHAP for variance has yet to be implemented!")
    ...
    def set_value_function(self, distributional_property, adjustment, model_function):
        ...
        if match:
            # Extract the percentage as an integer
            percentile = int(match.group(1))
            value_function = lambda instances: model_function(instances, adjustment=adjustment, percentile=[percentile])
        else:
            raise ValueError("Invalid distributional property! What Are We Explaining?")
